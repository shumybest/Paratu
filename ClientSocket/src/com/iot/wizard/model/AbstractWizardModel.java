...
public abstract class AbstractWizardModel implements ModelCallbacks {
    ...
    public AbstractWizardModel(Context context, Vector<ParatuDevice> devices) {
        mContext = context;
        mRootPageList = onNewRootPageList(devices);
    }
     ...
     * @param devices 
     */
    protected abstract PageList onNewRootPageList(Vector<ParatuDevice> devices);
    ...
    public Bundle save() {
        ...
        for (Page page : getCurrentPageSequence()) {
            bundle.putBundle(page.getKey(), page.getReviewData());
        }
        return bundle;
    }
