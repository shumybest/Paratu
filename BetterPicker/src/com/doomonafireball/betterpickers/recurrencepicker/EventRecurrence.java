...
public class EventRecurrence {
    ...
    private static String day2StringPerfect(int day) {
        switch (day) {
            case SU:
                return "星期天";
            case MO:
    ...
    private void appendByPerfectDay(StringBuilder s, int i) {
        int n = this.bydayNum[i];
        if (n != 0) {
            s.append(n);
        }

        String str = day2StringPerfect(this.byday[i]);
        s.append(str);
    }
    ...
    public String toPerfectString() {
        ...
        appendNumbers(s, "\n重复秒: ", this.bysecondCount, this.bysecond);
        appendNumbers(s, "\n重复分: ", this.byminuteCount, this.byminute);
        appendNumbers(s, "\n重复小时: ", this.byhourCount, this.byhour);
        ...
        if (count > 0) {
            ...
            count--;
            for (int i = 0; i < count; i++) {
            	appendByPerfectDay(s, i);
                s.append(",");
            }
            appendByPerfectDay(s, count);
        }

        appendNumbers(s, "\n重复月天: ", this.bymonthdayCount, this.bymonthday);
        appendNumbers(s, "\n重复年天: ", this.byyeardayCount, this.byyearday);
        appendNumbers(s, "\n重复周: ", this.byweeknoCount, this.byweekno);
        appendNumbers(s, "\n重复月: ", this.bymonthCount, this.bymonth);
        appendNumbers(s, "\n重复: ", this.bysetposCount, this.bysetpos);
